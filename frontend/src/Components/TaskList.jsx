...
const TaskList = () => {
  ...
  return (
    <>
    <Nav/>
  <div className=''>  <h1 className='text-3xl text-purple-500  font-extrabold  ml-52 -mt-[480px] text-center' >My ToDo Lists</h1></div>
    <div className="-mt-10 w-[850px] py-30 ml-72">
      {tasks.length > 0 ? (
        <ul className="grid grid-cols-2 gap-10">
          {tasks.map((task) => (
            <li key={task._id} className="border py-10 text-start text-black rounded-lg shadow-md relative">
              ...
              <p className="text-gray-500 text-sm  mr-6 text-end">{getFormattedTimeAgo(task.createdAt)}</p>
